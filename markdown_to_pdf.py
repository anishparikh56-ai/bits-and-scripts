...
def clean_markdown(content):
    # Remove YAML frontmatter and <a href="..."> links
    content = re.sub(r"^---[\s\S]*?---\n", "", content, flags=re.MULTILINE)
    content = re.sub(r'<a href=".*?">.*?</a>\n?', "", content, flags=re.MULTILINE)
    return content.strip()

def markdown_to_pdf(md_path, output_pdf_path):
    ...
    print(f"✅ Converted {md_path} → {output_pdf_path}")
...
def convert_all_index_md():
    ...
    for root, dirs, files in os.walk(base_dir):
        if 'index.md' in files:
            ...
            output_pdf = os.path.join(base_dir, f"{dir_name}.pdf")
            markdown_to_pdf(md_path, output_pdf)
